  ...
  public int onRunTask(TaskParams params){
    ...
    if (urlStringBuilder != null){
      ...
      try{
        ...
        Log.d(LOG_TAG, "getResponse - " + getResponse);
        result = GcmNetworkManager.RESULT_SUCCESS;
        try {
          ContentValues contentValues = new ContentValues();
          // update ISCURRENT to 0 (false) so new data is current
          if (isUpdate) {
            mContext.getContentResolver().delete(QuoteProvider.Quotes.CONTENT_URI, "1 = 1", null);
          }
          ...
          if (list.size() > 0) {
            ...
            Toast.makeText(mContext, mContext.getText(R.string.empty_stock_search_result), Toast.LENGTH_SHORT).show();
          }
